// multi-sort-flipkart-scraper.js
...
async function processPage(page, pageUrl, pageNum, sortOption, client) {
  console.log(`\n========= Processing ${sortOption} - page ${pageNum} of 25 =========\n`);
  ...
  page.on('response', async (response) => {
    ...
    if (
        (url.includes('/grocery/pr?sid=73z&marketplace=GROCERY') || url.includes(`&page=${pageNum}`)) &&
  ...
  if (stateData) {
        ...
        if (stateData?.pageDataV4?.page?.data) {
          ...
          for (const key of dataKeys) {
            ...
            if (Array.isArray(section)) {
              ...
              // Check if this object has numeric keys (like 1, 2, 3...)
    ...
    if (productData && productData.length > 0) {
      console.log(`Found ${productData.length} product groups on ${sortOption} - page ${pageNum}`);
      ...
      console.log(`${sortOption} - Page ${pageNum} data saved to database!`);

      // Commit transaction for this page
      await client.query('COMMIT');
    } else {
      console.log(`No products found on ${sortOption} - page ${pageNum}`);
      // Rollback empty transaction
      await client.query('ROLLBACK');
    }
  } else {
    console.log(`No state data was captured for ${sortOption} - page ${pageNum}`);
...
(async () => {
  ...
  try {
    ...
    for (const sortOption of sortOptions) {
      ...
      // Process each page from 1 to 2 (changed from 25 for testing)
    ...
    // After processing all pages and sorts, update brand product counts
    ...
    console.log('All data from all sort options saved successfully to database!');
